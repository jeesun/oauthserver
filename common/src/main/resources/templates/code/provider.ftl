    ...
    <#list columns as column>
        <#if column.allowSearch>
            <#switch column.type>
                ...
                <#break>
                <#case "Date">
                if (null != param.get("${column.name}Start") && null == param.get("${column.name}End")) {
                    WHERE("tb1.${column.name?replace("([a-z])([A-Z]+)","$1_$2","r")?lower_case} gt;=${r'#{' + column.name + 'Start}'}");
                }
                if (null == param.get("${column.name}Start") && null != param.get("${column.name}End")) {
                    WHERE("tb1.${column.name?replace("([a-z])([A-Z]+)","$1_$2","r")?lower_case} lt;=${r'#{' + column.name + 'End}'}");
                }
                if (null != param.get("${column.name}Start") && null != param.get("${column.name}End")) {
                    WHERE("tb1.${column.name?replace("([a-z])([A-Z]+)","$1_$2","r")?lower_case} BETWEEN ${r"#{" + column.name + "Start}"} AND ${r"#{" + column.name + "End}"}");
                }
                <#break>
                <#default>
